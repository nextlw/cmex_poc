...
async def direct_queries(request: Request):
    try:
        ...
        print(
            "[DEBUG] Dados formatados para validação:",
            {"consulta": consulta_produto.consulta, "modelo": consulta_produto.modelo},
        )

        print(
            f"[DEBUG] Modelo solicitado: {modelo_original}, mas usando modelo local: {modelo}"
        )
        ...
        try:
            ...
            if not funcao_escolhida:
                print(
                    f"[ERROR] Modelo {consulta_produto.modelo} não encontrado, tentando fallback"
                )
            ...
            try:
                ...
                try:
                    await supabase.table("pesquisas").insert(
                        novo_registro_pesquisas
                    ).execute()
                except Exception as e:
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=10.0) as client:
            try:
                ...
                return JSONResponse(
                    content={
                        "error": f"Erro ao verificar status: {task_response.status_code}"
                    },
...
async def cancel_process(request: Request):
    ...
    try:
        ...
        if not data.get("requestId"):
            return JSONResponse(
                status_code=400, content={"error": "ID da requisição é obrigatório"}
            )
        ...
        try:
            ...
            if res.data and len(res.data) > 0:
                ...
                print(
                    f"[Cancelamento] Pesquisa {request_id} marcada como cancelada no Supabase"
                )
        except Exception as e:
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                ...
                if node_response.status_code == 200:
                    print(
                        f"[Cancelamento] Solicitação de cancelamento enviada ao Node.js com sucesso"
                    )
                else:
                    print(
                        f"[Cancelamento] Falha ao solicitar cancelamento ao Node.js: {node_response.status_code}"
                    )
                    ...
                    print(
                        f"[Cancelamento] Resultado trash-query: {trash_response.status_code}"
                    )
        except Exception as e:
