...
class FrontendSimulator:
    ...
    def send_query(self, query, model="gpt-4"):
        ...
        try:
            ...
            response = requests.get(f"{FASTAPI_URL}/api/test/redis-integration", 
                                   params={"query": query},
                                   timeout=5)
            ...
            if response.status_code == 200:
                ...
                redis_client.publish(CHANNELS["NCM_REQUEST"], json.dumps({
    ...
    def check_status(self):
        ...
        try:
            response = requests.get(f"{FASTAPI_URL}/api/v1/query-status/{self.request_id}", timeout=5)
            if response.status_code == 200:
    ...
    def start_sse_listener(self):
        ...
        self.sse_thread = threading.Thread(target=self._sse_listener, args=(self.request_id,))
