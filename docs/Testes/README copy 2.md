...
O pacote fornece suporte para lidar com as diferenças nas respostas dos modelos de IA:
...
const processedResponse = isGeminiModel
  ? ModelResponse.extractJsonFromGeminiResponse(rawResponse)
  : rawResponse;

// Validar resposta processada
const validResponse = ModelResponse.responseSchema.safeParse(processedResponse);
```

### Funções Específicas para Gemini

O pacote inclui funções para processar respostas específicas do modelo Gemini:

```typescript style="background-color: #161921"
import { ModelResponse } from '@cmex/shared-types';

// Extrair JSON de uma resposta em formato markdown
const jsonData = ModelResponse.extractJsonFromGeminiResponse(`
\`\`\`json
{
...
// jsonData agora contém o objeto JSON extraído
```
...
### v1.0.1 (25/02/2025)
- Alteração na interface `LogsResponse`: o campo `logs` agora é obrigatório em vez de opcional
- Adicionado suporte para modelos Gemini, incluindo funções de extração de JSON de respostas markdown
- Correções em transformadores para garantir que o campo `logs` sempre esteja definido
- Melhorias na documentação com padronização visual dos blocos de código 