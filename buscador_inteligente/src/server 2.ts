...
import {StepAction, StreamMessage, TrackerContext, AnswerAction, OutputItem} from './types';
...
/**
 * Cria uma instância do OutputManager com persistência em disco
 */
const outputManager = new OutputManager(path.join(process.cwd(), 'outputs'));
...
function captureLLMOutput(requestId: string, type: string, data: any) {
  ...
  // Adiciona ao OutputManager
  outputManager.addAutoFormattedOutput(requestId, data, `Saída ${type}`);
  
  // Emite evento de atualização com os outputs recentes
...
function emitTrackerUpdate(requestId: string, context: TrackerContext) {
  ...
  // Obtenha os outputs do OutputManager
  const outputs = outputManager.getOutputs(requestId);
