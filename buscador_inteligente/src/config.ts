...
export function getToolConfig(toolName: ToolName): ModelConfig {
  const providerConfig = config.models[LLM_PROVIDER === 'vertex' ? 'gemini' : LLM_PROVIDER];
...
export function getModel(toolName: ToolName) {
  ...
  if (LLM_PROVIDER === 'vertex') {
    ...
    if (toolName === 'searchGrounding') {
      return createVertex({ project: process.env.GCLOUD_PROJECT, ...providerConfig?.clientConfig })(configTool.model, { useSearchGrounding: true });
    }
    return createVertex({ project: process.env.GCLOUD_PROJECT, ...providerConfig?.clientConfig })(configTool.model);
  }
  ...
  if (toolName === 'searchGrounding') {
    return createGoogleGenerativeAI({ apiKey: env.GEMINI_API_KEY })(configTool.model, { useSearchGrounding: true });
  }
  return createGoogleGenerativeAI({ apiKey: env.GEMINI_API_KEY })(configTool.model);
}

// Validação das variáveis de ambiente
if (LLM_PROVIDER === 'gemini' && !env.GEMINI_API_KEY) throw new Error("GEMINI_API_KEY not found");
if (LLM_PROVIDER === 'openai' && !env.OPENAI_API_KEY) throw new Error("OPENAI_API_KEY not found");
...
const configSummary = {
  ...
  tools: Object.fromEntries(
    Object.keys(config.models[LLM_PROVIDER === 'vertex' ? 'gemini' : LLM_PROVIDER].tools).map(name => [
      name,
