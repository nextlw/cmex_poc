...
async function tryWithRetry(
  ...
  try {
    ...
    if (options.maxRetries > 0) {
      ...
      console.log(
        `Tentativa falhou, tentando novamente em ${delay}ms... (${options.maxRetries} tentativas restantes)`
      );
      await sleep(delay);
...
export async function dedupQueries(
  ...
  // Se não houver queries existentes, retorna as queries únicas
  if (!existingQueries || existingQueries.length === 0) {
    return { unique_queries: uniqueQueries, tokens: 0 };
  }
  ...
  async function tryLocalModel() {
    ...
    try {
      ...
      // Garante que não há duplicatas na resposta do modelo
      const finalQueries = [...new Set(validated.unique_queries)];
      return {
        unique_queries: finalQueries,
        tokens: response.usageMetadata?.totalTokenCount || 0,
      };
    } catch (error) {
      console.error("Erro na validação do schema:", error);
      // Em caso de erro, retorna as queries únicas sem duplicatas exatas
      return {
        unique_queries: [...new Set(queries)],
        tokens: 0,
      };
    }
  }
