...
export class TextFormatter {
  ...
  private static processSegment(segment: string): ContentBlock[] {
    ...
    for (const paragraph of paragraphs) {
      ...
      if (trimmedParagraph.includes('|') && /\|[-:]+\|/.test(trimmedParagraph)) {
   ...
   */
  private static parseTable(text: string): { headers: string[], rows: string[][] } {
   ...
   */
  private static parseAlertLevel(text: string): 'info' | 'warning' | 'error' | 'success' {
    if (text.includes('warn') || text.includes('cuidado') || text.includes('atenção')) {
      return 'warning';
    } else if (text.includes('erro') || text.includes('perigo') || text.includes('falha')) {
      return 'error';
    } else if (text.includes('sucesso') || text.includes('ok') || text.includes('pronto')) {
      return 'success';
    } else {
      return 'info';
    }
