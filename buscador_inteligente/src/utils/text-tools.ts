...
const i18nTexts: Record<string, Record<string, string>> = {
  pt: {
    read_for: "Lendo as seguintes URLs para obter informações: {urls}",
    search_for: "Buscando informações sobre: {query}",
    reflect_on: "Refletindo sobre: {questions}",
    answer_for: "Respondendo à pergunta: {question}",
  },
  en: {
    read_for: "Reading the following URLs to gather information: {urls}",
    search_for: "Searching for information about: {query}",
    reflect_on: "Reflecting on: {questions}",
    answer_for: "Answering the question: {question}",
  },
};

/**
 * Obtém texto internacionalizado com base na chave e idioma
 *
 * @param key Chave do texto
 * @param languageCode Código do idioma (pt, en)
 * @param params Parâmetros para substituição no texto
 * @returns Texto traduzido com parâmetros substituídos
 */
export function getI18nText(
  ...
  // Obter o conjunto de textos para o idioma ou usar inglês como fallback
  const texts = i18nTexts[languageCode] || i18nTexts.en;

  // Obter o texto pela chave ou usar a própria chave como fallback
...
/**
 * Combina duas strings de forma inteligente, removendo duplicações
 *
 * @param str1 Primeira string
 * @param str2 Segunda string
 * @returns String combinada sem duplicações
 */
export function smartMergeStrings(
  str1: string = "",
  str2: string = ""
): string {
  if (!str1) return str2;
  ...
  // Se uma string é subconjunto da outra, retorna a maior
  if (clean1.includes(clean2)) return clean1;
  if (clean2.includes(clean1)) return clean2;

  // Encontra a maior sobreposição entre o final de str1 e o início de str2
  let maxOverlap = 0;
  for (let i = 1; i <= Math.min(clean1.length, clean2.length); i++) {
    const end1 = clean1.slice(-i);
    const start2 = clean2.slice(0, i);
    if (end1 === start2) {
      maxOverlap = i;
    }
  }

  // Combina as strings com base na sobreposição
  if (maxOverlap > 0) {
    return clean1 + clean2.slice(maxOverlap);
  }

  // Se não houver sobreposição, retorna as strings concatenadas com espaço
...
/**
 * Remove marcações HTML de um texto
 *
 * @param html Texto com marcações HTML
 * @returns Texto limpo sem marcações HTML
 */
...
/**
 * Trunca um texto para um tamanho máximo, preservando palavras inteiras
 *
 * @param text Texto a ser truncado
 * @param maxLength Tamanho máximo desejado
 * @param suffix Sufixo a ser adicionado quando o texto for truncado
...
export function truncateText(
  ...
  // Trunca no espaço mais próximo antes do maxLength
  ...
  // Se não houver espaço, trunca no maxLength exato
