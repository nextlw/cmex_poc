...
const FASTAPI_ENDPOINT = process.env.FASTAPI_ENDPOINT || 'http://localhost:8000/api/ncm';
 ...
 */
export async function ncmRouter(req: Request, res: Response, next: NextFunction): Promise<void> {
  ...
  const { consulta, estadoOrigem, operacao, regimeTributario, tributacao, modelo, useDeepResearch } = req.body;
  ...
  try {
    ...
    try {
      const fastApiResponse = await axios.post(FASTAPI_ENDPOINT, {
        consulta, estadoOrigem, operacao, regimeTributario, tributacao
      });
      ...
      console.log('Fluxo DeepResearch: Enviando para análise profunda com FastAPI data');
      ...
      if (useDeepResearch && (error.code === 'ECONNREFUSED' || error.code === 'ENOTFOUND')) {
        console.log('FastAPI indisponível, processando via DeepResearch diretamente');
