...
export const transformQueryStatus = (backendStatus: string): FrontendQueryStatus => {
...
export class StreamingService {
  ...
  connect(requestId: string): void {
    this.socket = new WebSocket(`ws://localhost:3000/api/v1/stream/${requestId}`);
...
describe('ModelSelector', () => {
  ...
  test('renderiza com o modelo padrão selecionado', () => {
    ...
    expect(screen.getByLabelText(/modelo de ia/i)).toHaveValue('qwen2.5-7b-instruct-1m');
  });
...
describe('Integração do Chat', () => {
  beforeEach(() => {
    ...
    (StreamingService as jest.Mock).mockImplementation(() => mockStreamingService);
  });
  ...
  test('fluxo completo de envio de consulta e recebimento de resposta', async () => {
    ...
    await waitFor(() => {
      const mockStreamingService = (StreamingService as jest.Mock).mock.results[0].value;
      expect(mockStreamingService.connect).toHaveBeenCalledWith('mock-request-id');
    });
...
describe('Chat Page', () => {
          ...
          setTimeout(() => {
            ...
            setTimeout(() => {
              this.onmessage({
                data: JSON.stringify({
                  type: 'answer',
                  data: { answer: 'TypeScript é uma linguagem de programação...' }
                })
              });
            }, 1000);
          }, 500);
  ...
  it('permite ao usuário enviar uma pergunta e ver a resposta', () => {
    ...
    cy.contains('TypeScript é uma linguagem de programação...').should('be.visible');
  });
});
```
