...
const HomePage: React.FC = () => {
  ...
  const [dropdownSelection, setDropdownSelection] = useState<SelectionData | null>(() => {
  ...
  const [deepResearchRequestId, setDeepResearchRequestId] = useState<string | null>(() => {
  ...
  useEffect(() => {
    localStorage.setItem('deepResearchRequestId', JSON.stringify(deepResearchRequestId));
  }, [deepResearchRequestId]);

  useEffect(() => {
    localStorage.setItem('isDeepResearchProcessing', JSON.stringify(isDeepResearchProcessing));
  ...
  useEffect(() => {
    if (isDeepResearchProcessing && deepResearchRequestId) {
      const checkStatus = async () => {
        try {
          const response = await axiosInstance.get(`/task-status/${deepResearchRequestId}`);
          ...
          if (response.data.completed) {
            ...
            for (const item of updatedNCM) {
              if (item.validacao_deepresearch?.requestId === deepResearchRequestId) {
            ...
            console.error("Erro de conexão com DeepResearch:", response.data.error);
            
            // Tenta novamente após 10 segundos para erros de conexão
            if (response.data.error === 'connect_error' || response.data.error === 'timeout') {
              ...
              for (const item of updatedNCM) {
                if (item.validacao_deepresearch?.requestId === deepResearchRequestId) {
                  item.validacao_deepresearch = {
                    ...item.validacao_deepresearch,
                    mensagem: response.data.currentAction || 'Erro de conexão com o serviço DeepResearch'
                  };
                }
              }
              setSugerirNCM(updatedNCM);
            ...
            for (const item of updatedNCM) {
              if (item.validacao_deepresearch?.requestId === deepResearchRequestId) {
                item.validacao_deepresearch = {
                  ...item.validacao_deepresearch,
                  mensagem: response.data.currentAction || 'Análise em andamento...'
                };
              }
            }
            setSugerirNCM(updatedNCM);
  ...
  const handleSearch = async (
    ...
    try {
      ...
      if (useDeepResearch && response.data && Array.isArray(response.data) && response.data.length > 0) {
        ...
        setErrorMessage("Erro inesperado ao processar a consulta. Tente novamente.");
        setSugerirNCM([]);
      }
    } catch (error: any) {
      ...
      if (error.response && error.response.data && error.response.data.error) {
        ...
        setErrorMessage("Erro de conexão. Verifique se o servidor está disponível.");
      } else {
        ...
        <div className="col-span-12 -mt-4">
          <form className="space-y-4" onSubmit={(e) => e.preventDefault()}>
              ...
              <div className="grid-container p-4 pb-2">
                <div className="col-span-12">
                  <div className="flex flex-wrap justify-between items-center gap-4">
                    ...
                    <div className="flex-shrink-0 ml-auto" style={{ minWidth: '180px' }}>
                      <DeepResearchToggle
            ...
            <div className="gap-4 space-y-4">
              ...
              {useDeepResearch && sugerirNCM.length > 0 && !isLoading && !isDeepResearchProcessing && (
              ...
              {sugerirNCM.map((item, index) => (
                <div key={index} className="box-page">
                  ...
                  {useDeepResearch && item.validacao_deepresearch && (
                    <div className={`px-4 py-3 mb-4 rounded-md text-sm font-medium flex items-start ${
                      ...
                      <div>
                        <div className="font-semibold mb-1">
                          {item.validacao_deepresearch.status === 'confirmado' && 'NCM Confirmado por DeepResearch'}
                          {item.validacao_deepresearch.status === 'negado' && 'NCM Contestado por DeepResearch'}
                          {item.validacao_deepresearch.status === 'sugestao' && 'Sugestão Alternativa por DeepResearch'}
                          {(item.validacao_deepresearch.status === 'erro' || item.validacao_deepresearch.status === 'timeout') && 'Validação DeepResearch Indisponível'}
                        </div>
                        <p className="text-sm font-normal">{item.validacao_deepresearch.mensagem}</p>
                      </div>
                    </div>
                  )}
